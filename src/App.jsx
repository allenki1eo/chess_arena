...
export default function App() {
  ...
  const checkAchs = useCallback((state) => {
    ...
    setNewAchs(gained);
    gained.forEach(a => { popup(`🏆 ${a.name}  +${a.xp} XP`, "#facc15"); track("achievement_unlocked", { id: a.id, name: a.name, xp: a.xp }); });
  ...
  const startBattle = useCallback(async (m) => {
    ...
    setScreen("intro");
    track("battle_started", { model: m.name, tier: m.tier, multiplier: m.mult });
...
.btn-play{font-family:var(--fdis);font-size:1rem;letter-spacing:.1em;padding:18px 48px;touch-action:manipulation;
...
/* ── GAME SCREEN ── */
.game-screen{display:grid;grid-template-columns:240px 1fr 200px;min-height:100dvh;position:relative;z-index:1;
...
.board-wrap{
  ...
  padding:10px;
  background:linear-gradient(135deg,#1a1208,#2a1e0a,#1a1208);
  border-radius:8px;
  box-shadow:0 8px 32px rgba(0,0,0,.9),inset 0 1px 0 rgba(255,255,255,.07);
  border:1px solid rgba(255,220,100,.08);
}
...
.coord{
  ...
  color:rgba(255,220,120,.45);font-weight:600;
}
...
.board{
  ...
  box-shadow:
    0 0 0 2px rgba(180,130,50,.35),
    0 0 0 4px rgba(0,0,0,.6),
    0 20px 80px rgba(0,0,0,.95),
    0 0 60px rgba(var(--c,100,80,30),.08);
}
.sq{
  width:60px;height:60px;display:flex;align-items:center;justify-content:center;
  position:relative;cursor:pointer;transition:background .12s,filter .12s;
  touch-action:manipulation;-webkit-tap-highlight-color:transparent;
}
.sq-l{
  background:radial-gradient(ellipse at 30% 30%,#f5deb3,#e8c98a);
}
.sq-d{
  background:radial-gradient(ellipse at 30% 30%,#a0714a,#7a5230);
}
.sq-l:hover{background:radial-gradient(ellipse at 30% 30%,#fdefc4,#f0d89c);filter:brightness(1.06)}
.sq-d:hover{background:radial-gradient(ellipse at 30% 30%,#b8845a,#8c6038);filter:brightness(1.1)}
.sq-sel{
  background:radial-gradient(ellipse at 30% 30%,#ffe566,#f5c030)!important;
  box-shadow:inset 0 0 0 2px rgba(255,255,255,.5),inset 0 0 12px rgba(255,220,50,.4);
}
.sq-last{
  background:rgba(245,197,66,.22)!important;
  box-shadow:inset 0 0 0 1px rgba(245,197,66,.35);
}
.sq-check{
  background:radial-gradient(ellipse at 50% 50%,rgba(239,68,68,.85),rgba(180,20,20,.7))!important;
  animation:glow 1s ease-in-out infinite;
}
/* Pieces */
.piece{
  font-size:2.6rem;line-height:1;user-select:none;pointer-events:none;z-index:2;position:relative;
  transition:transform .12s var(--eback);
}
.sq:hover .piece,.sq-sel .piece{transform:scale(1.12) translateY(-2px)}
/* Black (AI) pieces — deep graphite with subtle teal edge */
.pb{
  color:#1c1c2e;
  -webkit-text-stroke:1.5px rgba(80,200,255,.22);
  filter:
    drop-shadow(0 3px 6px rgba(0,0,0,1))
    drop-shadow(0 1px 2px rgba(0,0,0,1))
    drop-shadow(0 0 4px rgba(40,160,220,.15));
}
/* White (player) pieces — warm ivory with golden edge */
.piece:not(.pb){
  color:#fdf0d5;
  -webkit-text-stroke:1px rgba(120,60,0,.55);
  filter:
    drop-shadow(0 3px 6px rgba(0,0,0,.9))
    drop-shadow(0 1px 3px rgba(0,0,0,.8))
    drop-shadow(0 0 5px rgba(255,200,80,.18));
}
/* Move dot — subtle dark circle */
.dot{
  position:absolute;width:22px;height:22px;border-radius:50%;
  background:rgba(0,0,0,.28);pointer-events:none;z-index:1;
  box-shadow:0 1px 3px rgba(0,0,0,.4);
}
/* Capture ring — vivid red ring */
.ring{
  position:absolute;inset:4px;border-radius:50%;
  border:4px solid rgba(239,68,68,.55);
  pointer-events:none;z-index:1;
  box-shadow:0 0 8px rgba(239,68,68,.3),inset 0 0 4px rgba(239,68,68,.15);
}
...
@media(max-width:1100px){
  .home-screen{grid-template-columns:1fr 300px}
  .game-screen{grid-template-columns:180px 1fr 160px}
}
...
@media(max-width:900px){
  ...
  .board-ranks .coord{height:52px}
  .board-files .coord{width:52px}
...
@media(max-width:600px){
  /* Home hero */
  .home-hero{padding:32px 20px;gap:14px}
  .hero-title{font-size:clamp(2.4rem,11vw,3.8rem)}
  .hero-sub{font-size:.9rem}
  .hero-stats{gap:16px}
  .hs-val{font-size:1rem}
  .btn-play{padding:14px 32px;font-size:.85rem}

  /* Header */
  .hdr{padding:0 12px;height:52px}
  .hdr-title{font-size:.82rem;letter-spacing:.06em}
  .hdr-sub{display:none}
  .xp-bar-wrap{display:none}
  .hdr-div{display:none}
  .hdr-stat:not(:last-child){display:none}

  /* Profile button */
  .pb-name{display:none}
  .pb-level{display:none}
  .profile-btn{padding:5px 8px;border-radius:50%;top:10px;right:12px}

  /* Select screen */
  .select-screen{padding:14px}
  .sel-header{flex-direction:column;align-items:flex-start;gap:10px}
  .sel-title{font-size:1.1rem}
  ...
  .sq{
    width:calc((100vw - 48px)/8);
    height:calc((100vw - 48px)/8);
  }
  .board-ranks .coord{height:calc((100vw - 48px)/8)}
  .board-files .coord{width:calc((100vw - 48px)/8)}
  .piece{font-size:calc((100vw - 48px)/8 * 0.62)}
  .dot{width:14px;height:14px}

  /* Villain HUD */
  .villain-hud{max-width:100%}
  .vh-name{font-size:.72rem}
  .vh-speech{font-size:.75rem}
  .gmb-sub{display:none}

  /* Intro screen */
  .intro-card{padding:36px 24px}
  .intro-avatar{font-size:3.5rem}
  .intro-label{font-size:1.3rem}

  /* Game over overlay */
  .ov{padding:28px 18px;max-width:95vw}
  .ov-result{font-size:1.8rem}
  .ov-pts{font-size:2.4rem}
  .ov-rows{gap:4px}
  .ov-row{padding:5px 10px;font-size:.78rem}
  .ov-actions{flex-direction:column;gap:8px}
  .ov-actions button{width:100%}

  /* Onboard */
  .ob-card{padding:36px 22px}
  .ob-title{font-size:1.8rem}
  .ob-features{gap:6px}

  /* BYOK */
  .byok-toggle{font-size:.75rem;padding:9px 12px}
}
...
@media(max-width:380px){
  ...
  .board{
    grid-template-columns:repeat(8,calc((100vw - 36px)/8));
    grid-template-rows:repeat(8,calc((100vw - 36px)/8));
  }
  .sq{
    width:calc((100vw - 36px)/8);
    height:calc((100vw - 36px)/8);
  }
  .board-ranks .coord{height:calc((100vw - 36px)/8)}
  .board-files .coord{width:calc((100vw - 36px)/8)}
  .piece{font-size:calc((100vw - 36px)/8 * 0.6)}
